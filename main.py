    ...
    def run(self,
        ...
        try:
            ...
            while True:
                ...
                if self.is_continue:
                    # print('eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee12312321312321321')
                    # print(next(dataset))
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        # self.det_thread.send_visual.connect(self.visualize_result)
        ...
        self.saveCheckBox2.clicked.connect(self.visualize_result)
    ...
    def update_plot(self):
        ...
        self.ydata = self.ydata[1:] + [self.num_gum]
        
        self.canvas.axes.cla()  # Clear the canvas.

        self.canvas.axes.plot(self.xdata, self.ydata, color='#08F7FE',label = 'Gum')
        
        self.ydata2 = self.ydata2[1:] + [self.num_metos]
        self.canvas.axes.plot(self.xdata, self.ydata2,  color='#FE53BB',label = 'Metos')
        
        self.ydata3 = self.ydata3[1:] + [self.num_pure]
        self.canvas.axes.plot(self.xdata, self.ydata3,  color='#F5D300',label = 'Pure')
    ...
    def show_statistic(self, statistic_dic):
        try:
            ...
            labels = [str(i[0]) for i in statistic_dic]
            values = [i[1] for i in statistic_dic]
            try: 
                self.num_gum = values[labels.index('Gum')]
            except:
                self.num_gum = 0
            try: 
                self.num_metos = values[labels.index('Mentos')]
            except:
                self.num_metos = 0
            try: 
                self.num_pure = values[labels.index('Pure')]
            except:
                self.num_pure = 0  
            # send to event Hub
            try:
                ...
                reading = {'Gum': self.num_gum,'Mentos': self.num_metos,'Pure': self.num_pure}
